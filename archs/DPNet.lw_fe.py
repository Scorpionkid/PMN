...
class DPNet_lw_fe(nn.Module):
    ...
    def __init__(self, args=None, texture_params=None, **kwargs):
        ...
        self.enable_intermediate_supervision = args['enable_intermediate_supervision']
        ...
        # 🆕 新增：多层频域增强模块
        # 第1层频域增强
        self.freq_enhance_detail_1 = Freq(base_channels)
        self.freq_enhance_denoise_1 = Freq(base_channels)
        
        # 第2层频域增强  
        self.freq_enhance_detail_2 = Freq(base_channels*2)
        self.freq_enhance_denoise_2 = Freq(base_channels*2)
    ...
    def forward(self, x, noise_map=None, texture_mask=None):
        ...
        enc1_detail_enhanced = enc1_detail + self.freq_enhance_detail_1(enc1_detail)  # 新增频域增强
        enc1_detail_down = self.down(enc1_detail_enhanced)
        
        enc1_denoise = self.enc1_denoise(x, nm_original, tm_original)
        enc1_denoise_enhanced = enc1_denoise + self.freq_enhance_denoise_1(enc1_denoise)  # 新增频域增强
        enc1_denoise_down = self.down(enc1_denoise_enhanced)

        # 🆕 第2层编码 + 频域增强
        enc2_detail = self.enc2_detail(enc1_detail_down, nm_down1, tm_down1)
        enc2_detail_enhanced = enc2_detail + self.freq_enhance_detail_2(enc2_detail)  # 新增频域增强
        enc2_detail_down = self.down(enc2_detail_enhanced)
        
        enc2_denoise = self.enc2_denoise(enc1_denoise_down, nm_down1, tm_down1)
        enc2_denoise_enhanced = enc2_denoise + self.freq_enhance_denoise_2(enc2_denoise)  # 新增频域增强
        enc2_denoise_down = self.down(enc2_denoise_enhanced)
        ...
        #--------------------------- 解码器路径（保持原有逻辑：跳跃连接求平均） ---------------------------#

        # 解码器3 - 注意使用增强后的特征进行跳跃连接
        ...
        # 🔄 修改：跳跃连接使用增强后的特征求平均
        dec2_input = torch.cat([dec3_up, (enc2_detail_enhanced + enc2_denoise_enhanced)/2], dim=1)
        ...
        # 🔄 修改：跳跃连接使用增强后的特征求平均
        dec1_input = torch.cat([dec2_up, (enc1_detail_enhanced + enc1_denoise_enhanced)/2], dim=1)
