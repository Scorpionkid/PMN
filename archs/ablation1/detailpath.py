...
class DPNet_onlydetail(nn.Module):
    ...
    def forward(self, x, noise_map=None, texture_mask=None):
        ...
        # 然后使用增强后的特征
        bottleneck_input = enc3_detail_enhanced
        ...
        # 将跳跃连接从两条独立路径连接
        dec3_input = torch.cat([bottleneck_up, enc3_detail], dim=1)
        ...
        dec2_input = torch.cat([dec3_up, enc2_detail], dim=1)
        ...
        dec1_input = torch.cat([dec2_up, enc1_detail], dim=1)
