            ...
            with tqdm(total=len(self.dataloader_train)) as t:                
                for k, data in enumerate(self.dataloader_train):
                    ...
                    if 'DPNet' in self.arch['name']:
                        loss = 0.0  # 初始化总损失
                        loss_values = {}  # 初始化损失值字典
                        if noise_map is not None:
                            pred = self.net(imgs_lr, noise_map)
                            if self.dst['ori'] is True:
                                pred = pred * ratio
                            # 检查输出格式
                            if 'detail' in self.arch['name']:
                                # 可以添加VGG感知损失，需要先初始化
                                if hasattr(self, 'perceptual_loss'):
                                    detail_percep_loss = self.perceptual_loss(pred, imgs_hr)
                                    loss += detail_percep_loss * 0.1
                                    loss_values['detail_percep_loss'] = detail_percep_loss.item()
                                    
                                # 添加梯度损失
                                if hasattr(self, 'gradient_loss'):
                                    detail_grad_loss = self.gradient_loss(pred, imgs_hr)
                                    loss += detail_grad_loss * 0.5  # 权重可调
                                    loss_values['detail_grad_loss'] = detail_grad_loss.item()
                            
                            else:
                                denoise_loss, _ = self.l1loss(pred, imgs_hr)
                                loss += denoise_loss * 0.5  # 权重可调
