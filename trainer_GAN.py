...
from archs.discriminator_arch import (
    MultiScaleVGGFreqDiscriminator,
    SimpleMultiScaleDiscriminator,
    GANLoss,
    gradient_penalty
)
...
class SID_Trainer(Base_Trainer):
    ...
    def _init_gan_components(self):
            """在__init__()末尾调用此方法初始化GAN组件"""
            
            # 检查是否启用对抗训练
            if not self.args['loss'].get('adversarial', False):
                self.use_gan = False
                return
            
            self.use_gan = True
            log("Initializing GAN components for adversarial training...")
            
            # 1. 创建判别器
            disc_config = self.args['discriminator']
            disc_type = disc_config.get('type', 'multiscale_vgg_freq')
            
            if disc_type == 'multiscale_vgg_freq':
                self.discriminator = MultiScaleVGGFreqDiscriminator(
                    in_channels=disc_config.get('in_channels', 4),
                    nf=disc_config.get('nf', 64),
                    num_scales=disc_config.get('num_scales', 3),
                    vgg_layers=disc_config.get('vgg_layers', [2, 7, 12, 21, 30]),
                    use_freq_discriminator=disc_config.get('use_freq_discriminator', True)
                ).to(self.device)
            else:  # simple_multiscale
                self.discriminator = SimpleMultiScaleDiscriminator(
                    in_channels=disc_config.get('in_channels', 4),
                    nf=disc_config.get('nf', 64),
                    num_scales=disc_config.get('num_scales', 3)
                ).to(self.device)
            ...
            # 2. GAN损失
            gan_config = self.args['loss'].get('gan', {})
            self.criterion_gan = GANLoss(
                gan_type=gan_config.get('type', 'vanilla'),
                real_label_val=gan_config.get('real_label_val', 1.0),
                fake_label_val=gan_config.get('fake_label_val', 0.0)
            ).to(self.device)
            
            # 3. 判别器优化器
            self.optimizer_d = torch.optim.Adam(
                self.discriminator.parameters(),
                lr=self.args['hyper'].get('learning_rate_d', 1e-4),
                betas=(0.5, 0.999)
            )
            
            # 4. 特征匹配损失
            self.use_feature_matching = self.args.get('feature_matching', {}).get('enabled', False)
            if self.use_feature_matching:
                self.criterion_fm = nn.L1Loss()
            
            # 5. GAN训练配置
            self.gan_config = self.args['hyper'].get('gan_training', {})
            self.n_critic = self.gan_config.get('n_critic', 1)
            self.adv_schedule = self.args['loss'].get('adversarial_schedule', {})
            self.loss_weights = self.args['loss'].get('weights', {})
            
            # 6. 损失记录
            self.d_loss_meter = AverageMeter('D_Loss', ':.4f')
            self.g_adv_loss_meter = AverageMeter('G_Adv_Loss', ':.4f')
            
            log("GAN components initialized successfully!")
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.hyper['stop_epoch']+1):
                    ...
                    if 'DPNet' in self.arch['name']:
                        if noise_map is not None:
                            ...
                            if isinstance(outputs, tuple) and len(outputs) == 4:
                                ...
                                # 1. 训练判别器
                                if self.use_gan:
                                    adv_weight = self.get_current_adversarial_weight()
                                    
                                    if adv_weight > 0:
                                        # 训练判别器 n_critic 次
                                        for _ in range(self.n_critic):
                                            d_loss = self.train_discriminator_step(imgs_hr, main_output)
                                ...
                                # 添加对抗损失
                                if self.use_gan and adv_weight > 0:
                                    # 对抗损失
                                    adv_loss = self.compute_adversarial_loss(main_output)
                                    loss = loss + adv_loss * adv_weight
                                    self.g_adv_loss_meter.update(adv_loss.item())
                                    loss_values['adv_loss'] = adv_loss.item()
                                    
                                    # 特征匹配损失
                                    if self.use_feature_matching:
                                        fm_loss = self.compute_feature_matching_loss(imgs_hr, main_output)
                                        if fm_loss > 0:
                                            fm_weight = self.args.get('feature_matching', {}).get('weight', 10.0)
                                            loss = loss + fm_loss * fm_weight
                                            loss_values['fm_loss'] = fm_loss.item() if isinstance(fm_loss, torch.Tensor) else fm_loss
            ...
            # ✅ epoch结束时记录（推荐）
            if self.use_gan:
                self.log_gan_info(epoch, len(self.dataloader_train))
    ...
    def compute_multi_loss_forUnet(self, main_output, gt):
        ...
        # 可以添加VGG感知损失，需要先初始化
        if hasattr(self, 'perceptual_loss'):
            detail_percep_loss = self.perceptual_loss(main_output, gt)
            total_loss += detail_percep_loss * 0.1  # 权重可调
            loss_values['percep_loss'] = detail_percep_loss.item()
            
        # 添加梯度损失
        if hasattr(self, 'gradient_loss'):
            detail_grad_loss = self.gradient_loss(main_output, gt)
            total_loss += detail_grad_loss * 0.5  # 权重可调
            loss_values['grad_loss'] = detail_grad_loss.item()
    ...
    def get_current_adversarial_weight(self):
        """根据当前epoch计算对抗损失权重"""
        if not self.use_gan or not self.adv_schedule.get('enabled', True):
            return 0.0
        
        warmup_epochs = self.adv_schedule.get('warmup_epochs', 50)
        rampup_epochs = self.adv_schedule.get('rampup_epochs', 100)
        max_weight = self.adv_schedule.get('max_weight', 0.05)
        min_weight = self.adv_schedule.get('min_weight', 0.001)
    ...
    def train_discriminator_step(self, real_imgs, fake_imgs):
        """
        训练判别器一步
        
        Args:
            real_imgs: 真实图像 [B, 4, H, W]
            fake_imgs: 生成图像 [B, 4, H, W] (已detach)
        ...
        # 遍历多尺度输出
        for scale_name in real_pred.keys():
            for output_name in real_pred[scale_name].keys():
                # 真实样本损失
                d_real_loss += self.criterion_gan(
                    real_pred[scale_name][output_name], 
                    target_is_real=True
                )
                
                # 假样本损失
                d_fake_loss += self.criterion_gan(
                    fake_pred[scale_name][output_name],
                    target_is_real=False
                )
                count += 1
        
        # 总损失
        d_real_loss = d_real_loss / count
        d_fake_loss = d_fake_loss / count
        d_loss = (d_real_loss + d_fake_loss) * 0.5
        
        # 梯度惩罚 (WGAN-GP)
        if self.gan_config.get('use_gradient_penalty', False):
            gp = gradient_penalty(self.discriminator, real_imgs, fake_imgs, self.device)
            gp_weight = self.gan_config.get('gp_weight', 10.0)
        ...
        # 梯度裁剪
        if self.gan_config.get('clip_grad_norm', 0) > 0:
            torch.nn.utils.clip_grad_norm_(
                self.discriminator.parameters(),
                self.gan_config['clip_grad_norm']
            )
    ...
    def compute_adversarial_loss(self, fake_imgs):
        """
        计算生成器的对抗损失
        
        Args:
            fake_imgs: 生成图像 [B, 4, H, W]
        ...
        for scale_name in fake_pred.keys():
            for output_name in fake_pred[scale_name].keys():
                adversarial_loss += self.criterion_gan(
                    fake_pred[scale_name][output_name],
                    target_is_real=True  # 生成器希望判别器认为是真的
                )
                count += 1

        adversarial_loss = adversarial_loss / count
    ...
    def compute_feature_matching_loss(self, real_imgs, fake_imgs):
        ...
        # 提取真实图像特征
        with torch.no_grad():
            real_features = self.discriminator(real_imgs)
        
        # 提取生成图像特征
        fake_features = self.discriminator(fake_imgs)
        
        # 计算特征差异
        fm_loss = 0
        fm_config = self.args.get('feature_matching', {})
        target_layers = fm_config.get('layers', ['layer_2', 'layer_7', 'layer_12'])
        
        count = 0
        for scale_name in real_features.keys():
            for output_name in real_features[scale_name].keys():
                # 只使用指定的层
                layer_matched = any(layer in output_name for layer in target_layers)
                if layer_matched:
                    fm_loss += self.criterion_fm(
                        fake_features[scale_name][output_name],
                        real_features[scale_name][output_name]
                    )
                    count += 1
        
        if count > 0:
            fm_loss = fm_loss / count
        
        return fm_loss


    def save_gan_checkpoint(self, epoch, is_best=False):
        """
        保存GAN检查点 (在原save_model基础上扩展)
        
        在原有的save_model()方法中添加判别器状态
        """
        checkpoint = {
            'epoch': epoch,
    ...
    def load_gan_checkpoint(self, checkpoint_path):
        """
        加载GAN检查点 (在原load_model基础上扩展)
        
        在原有的load_model()方法中添加判别器加载
        """
        checkpoint = torch.load(checkpoint_path, map_location=self.device)
        
        # 加载生成器
        self.net = load_weights(self.net, checkpoint['model_state_dict'], 
                            multi_gpu=self.multi_gpu, by_name=True)
        ...
        if self.use_gan and 'discriminator_state_dict' in checkpoint:
            ...
            log("Discriminator loaded successfully!")

    def log_gan_info(self, epoch, step):
        """在训练日志中添加GAN相关信息"""
        ...
        log_str = (
            f"[GAN] Epoch: {epoch}, Step: {step}, "
            f"D_Loss: {self.d_loss_meter.avg:.4f}, "
            f"G_Adv_Loss: {self.g_adv_loss_meter.avg:.4f}, "
            f"Adv_Weight: {adv_weight:.4f}"
        )
        
        log(log_str, log=self.logfile)
        
        # TensorBoard记录
        if hasattr(self, 'writer') and self.writer is not None:
            global_step = epoch * len(self.dataloader_train) + step
            self.writer.add_scalar('GAN/D_Loss', self.d_loss_meter.avg, global_step)
            self.writer.add_scalar('GAN/G_Adv_Loss', self.g_adv_loss_meter.avg, global_step)
            self.writer.add_scalar('GAN/Adv_Weight', adv_weight, global_step)
